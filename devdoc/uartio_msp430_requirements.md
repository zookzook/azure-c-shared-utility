...
### uartio_clone_option
`uartio_clone_option()` is the implementation provided to the option handler instance created as part of `uartio_retrieveoptions()`.
```c
void * uartio_clone_option (const char * option_name, const void * option_value)
```

#### OPTIONHANDLER Interface Contract
**SRS_UARTIO_27_009: [** If the `option_name` argument is NULL, `uartio_clone_option()` shall return `NULL`. **]**  
**SRS_UARTIO_27_010: [** If the `option_value` argument is NULL, `uartio_clone_option()` shall return `NULL`. **]**  
**SRS_UARTIO_27_011: [** If the `option_name` argument indicates an option that is not handled by `uartio`, then `uartio_clone_option()` shall return `NULL`. **]**  
...
**SRS_UARTIO_27_102: [** `uartio_close()` shall await any outstanding bytes by polling `EUSCI_A_UART_queryStatusFlags()` until it returns `false`. **]**  
...
#### XIO Interface Contract
**SRS_UARTIO_27_016: [** `uartio_create()` shall create a new instance of the uartio for MSP430. **]**  
**SRS_UARTIO_27_017: [** If `io_create_parameters()` is `NULL`, `uartio_create()` shall fail and return `NULL`. **]**  
**SRS_UARTIO_27_018: [** If allocating memory for the UART IO instance fails then `uartio_create()` shall fail and return `NULL`. **]**  
**SRS_UARTIO_27_019: [** When `uartio_create()` fails, all allocated resources up to that point shall be freed. **]**  

#### UARTIO Specific Implmentation
**SRS_UARTIO_27_105: [** If the argument the stored singleton value is non-`NULL`, then `uartio_create()` shall fail and return `NULL`. **]**  
**SRS_UARTIO_27_106: [** If the `baud_rate` member of the `UARTIO_CONFIG *` parameters is ZERO, then `uartio_create()` shall fail and return `NULL`. **]**  
**SRS_UARTIO_27_107: [** If the `ring_buffer_size` member of the `UARTIO_CONFIG *` parameters is ZERO, then `uartio_create()` shall fail and return `NULL`. **]**  
**SRS_UARTIO_27_108: [** `uartio_create()` shall store the `baud_rate` value for later use when enabling the underlying UART. **]**  
**SRS_UARTIO_27_109: [** `uartio_create()` shall store the `ring_buffer_size` value rounded up to the next power of two, minus one, as an index mask. **]**  
...
**SRS_UARTIO_27_112: [** If `uartio_create()` succeeds, then the instance value will be stored as the singleton value. **]**  
...
#### XIO Interface Contract
**SRS_UARTIO_27_020: [** `uartio_destroy()` shall free the uartio MSP430 instance. **]**  
...
#### UARTIO Specific Implmentation
**SRS_UARTIO_27_113: [** If the argument `io_handle` is NOT equal to stored singleton value, `uartio_destroy()` shall fail and return a non-zero value. **]**  
**SRS_UARTIO_27_114: [** `uartio_destroy()` shall set the singleton to `NULL`. **]**  
...
#### UARTIO Specific Implmentation
**SRS_UARTIO_27_116: [** If the argument `io_handle` is NOT equal to stored singleton value, `uartio_dowork()` shall fail and return a non-zero value. **]**  
...
**SRS_UARTIO_27_121: [** `uartio_dowork()` shall await any outstanding bytes by polling `EUSCI_A_UART_getInterruptStatus()` until it returns `false`. **]**  
...
**SRS_UARTIO_27_033: [** `uartio_open()` shall open the UART io and on success it shall return ZERO. **]**  
...
**SRS_UARTIO_27_041: [** If `buffer_size` is ZERO, `uartio_send()` shall fail and return a non-zero value. **]**  
**SRS_UARTIO_27_042: [** `uartio_send()` shall send `buffer_size` bytes, pointed to by `buffer`, and on success it shall return ZERO. **]**  
