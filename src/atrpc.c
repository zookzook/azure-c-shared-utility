...
typedef struct ATRPC_INSTANCE_TAG {
    ...
    size_t result_code_machine_state;
    ATRPC_STATUS status;
...
static bool autobaud_negotiation_machine (ATRPC_HANDLE handle_, unsigned char byte_) {
    ...
    switch (handle_->result_code_machine_state) {
      case 0:
        response_complete = false;
        if ('\r' == byte_) { handle_->result_code_machine_state = 1; }
        break;
      case 1:
        response_complete = false;
        if ('0' == byte_) { handle_->result_code_machine_state = 2; }
        else if ('\r' == byte_) { handle_->result_code_machine_state = 4; }
        else if ('\n' == byte_) { handle_->result_code_machine_state = 3; }
        else { handle_->result_code_machine_state = 0; }
        break;
      case 2:
        response_complete = ('\r' == byte_);
        handle_->result_code_machine_state = 0;
        break;
      case 3:
        response_complete = false;
        if ('0' == byte_) { handle_->result_code_machine_state = 2; }
      ...
      case 4:
        response_complete = false;
        if ('0' == byte_) { handle_->result_code_machine_state = 2; }
        else if ('\r' == byte_) { /* defer */ }
        else if ('\n' == byte_) { handle_->result_code_machine_state = 5; }
        else { handle_->result_code_machine_state = 0; }
        break;
      case 5:
        response_complete = false;
        if ('0' == byte_) { handle_->result_code_machine_state = 2; }
        else if ('O' == byte_) { handle_->result_code_machine_state = 6; }
        else if ('\r' == byte_) { handle_->result_code_machine_state = 4; }
        else { handle_->result_code_machine_state = 0; }
        break;
      case 6:
        response_complete = false;
        if ('K' == byte_) { handle_->result_code_machine_state = 7; }
        else { handle_->result_code_machine_state = 0; }
        break;
      case 7:
        response_complete = false;
        if ('\r' == byte_) { handle_->result_code_machine_state = 8; }
        else { handle_->result_code_machine_state = 0; }
        break;
      case 8:
        response_complete = ('\n' == byte_);
...
static void modem_handshake (void * context_, TA_RESULT_CODE result_code_, const char * response_) {
    (void)response_;
        ...
        if (50 < atrpc->handshake_attempt) {
            ...
            switch (atrpc->handshake_machine_state) {
              case 0:
                ...
                if (0 != atrpc_attention(context_, NULL, 0, handshake_timeout, modem_handshake, context_)) {
              ...
              case 1:
                ...
                /* Codes_SRS_ATRPC_27_061: [ When auto-baud negotiation has completed, then modem_on_bytes_received() shall normalize the ta responses by calling (int)xio_send(XIO_HANDLE handle, const void * buffer, size_t size, ON_IO_SEND_COMPLETE on_io_send_complete, void * on_io_send_context) using the xio handle returned from xio_create() during atrpc_create() for the handle parameter, and "ATE1V0\r" for the buffer parameter, and "7" for the size parameter. ] */
                  if (0 != atrpc_attention(context_, echo_enabled_verbosity_disabled, (sizeof(echo_enabled_verbosity_disabled) - 1), handshake_timeout, modem_handshake, context_)) {
              ...
              case 2:
                ...
                if (0 != atrpc_attention(context_, write_active_profile, (sizeof(write_active_profile) - 1), handshake_timeout, modem_handshake, context_)) {
...
static void modem_on_bytes_received (void * context_, const unsigned char * buffer_, size_t size_) {
    ...
    if (NULL == context_) {
        ...
        if (ATRPC_CLOSED == atrpc->status) {
            ...
            bool response_complete = false, buffer_error = false;
            size_t index = 0;
            for (; index < size_ && !response_complete; ++index) {
                ...
                if (ATRPC_NEGOTIATING_AUTOBAUD == atrpc->status) {
                    ...
                    /* Codes_SRS_ATRPC_27_053: [ If the on_open_complete() callback has been called, then modem_on_bytes_received() shall capture any bytes following the prefix of the command_string parameter passed to attention() along with the postfixed <result code>"\r". ] */
                    if (atrpc->awaiting_echo) {
                        if (atrpc->current_request[atrpc->echo_machine_state] == received_byte) {
                            ...
                            if (atrpc->current_request_length == atrpc->echo_machine_state) {
                                ...
                                atrpc->result_code_machine_state = 1;
                            }
                        } else {
                            atrpc->echo_machine_state = 0;
                        }
                    } else {
                        if (0 != VECTOR_push_back(atrpc->response, &received_byte, 1)) {
                            /* Codes_SRS_ATRPC_27_055: [ If an error occurs when capturing the bytes, then modem_on_bytes_received() shall call the ta_response callback passed to attention() using the ta_response_context as the context parameter, the ERROR_ATRPC as the result_code parameter, and the captured message as the message parameter. ] */
                            buffer_error = true;
                            response_complete = true;
                        } else {
                            switch (atrpc->result_code_machine_state) {
                              case 0:
                                if ('\r' == received_byte) { atrpc->result_code_machine_state = 1; }
                                break;
                              case 1:
                                // Accept all valid result codes
                                if (0x35 != received_byte && 0x30 <= received_byte && 0x39 >= received_byte) {
                                    atrpc->result_code_machine_state = 2;
                                }
                                else if ('\r' == received_byte) { /* defer */ }
                                else if ('\n' == received_byte) { atrpc->result_code_machine_state = 3; }
                                else { atrpc->result_code_machine_state = 0; }
                                break;
                              case 2:
                                if ('\r' == received_byte) {
                                    response_complete = true;
                                }
                                atrpc->result_code_machine_state = 0;
                                break;
                              case 3:
                                // Accept all valid result codes
                                if (0x35 != received_byte && 0x30 <= received_byte && 0x39 >= received_byte) {
                                ...
                                else if ('\r' == received_byte) { atrpc->result_code_machine_state = 1; }
                                else { atrpc->result_code_machine_state = 0; }
                                break;
                            }
                        }
                ...
                if (response_complete) {
                    ...
                    /* Codes_SRS_ATRPC_27_054: [ If any bytes where captured, modem_on_bytes_received() shall call the ta_response callback passed to attention() using the ta_response_context as the context parameter, the captured result code as the result_code parameter, and the captured message as the message parameter. ] */
...
int atrpc_attention (ATRPC_HANDLE handle_, const unsigned char * command_string_, size_t command_string_length_, size_t timeout_ms_, ON_TA_RESPONSE on_ta_response_, void * on_ta_response_context_) {
    int result;
...
ATRPC_HANDLE atrpc_create (void) {
    ...
    /* Codes_SRS_ATRPC_27_028: [ atrpc_create() shall create a vector to buffer responses from the terminal adapter. ] */
    } else if (NULL == (result->response = VECTOR_create(sizeof(unsigned char)))) {
        /* Codes_SRS_ATRPC_27_029: [ If the call to VECTOR_create() returns NULL, then atrpc_create() shall fail and return NULL. ] */
        xio_destroy(result->modem_io);
        tickcounter_destroy(result->tick_counter);
        free(result);
        result = NULL;
        LogError("atrpc_create() unable to create response vector!");
    } else {
...
void atrpc_destroy (ATRPC_HANDLE handle_) {
    if (NULL == handle_) {
        ...
        /* Codes_SRS_ATRPC_27_035: [ atrpc_destroy() shall call (void)VECTOR_destroy(VECTOR_HANDLE handle) using the handle returned from the call to VECTOR_create() for the handle parameter. ] */
        VECTOR_destroy(handle_->response);
...
void atrpc_dowork (ATRPC_HANDLE handle_) {
    if (NULL == handle_) {
        ...
        /* Codes_SRS_ATRPC_27_041: [ If atrpc_open() has been called on the handle, atrpc_dowork() shall mark the call time, by calling (int)tickcounter_get_current_ms(TICKCOUNTER_HANDLE handle, tickcounter_ms_t * current_ms) using the handle returned from atrpc_create() as the handle parameter. ] */
            ...
            handle_->on_ta_response(handle_->on_ta_response_context, ERROR_ATRPC, NULL);
        }
    }
}
...
int atrpc_open (ATRPC_HANDLE handle_, ON_OPEN_COMPLETE on_open_complete_, void * on_open_complete_context_) {
    ...
    if (0 != result && NULL != on_open_complete_) { on_open_complete_(on_open_complete_context_, ERROR_ATRPC); }
