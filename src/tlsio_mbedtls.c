...
// Must be called by this name to resolve extern from MBED TLS library
...
static int mbedtls_init(void *instance, const char *host) {
    TLS_IO_INSTANCE *tlsioInst = (TLS_IO_INSTANCE *)instance;
	int result = 0;
    char *pers = "azure_iot_client";

    // mbedTLS initialize...
    mbedtls_entropy_init(&tlsioInst->entropy);	
    mbedtls_ctr_drbg_init(&tlsioInst->ctr_drbg);
    mbedtls_ssl_init(&tlsioInst->ssl);			
    mbedtls_ssl_session_init(&tlsioInst->ssn);		
    mbedtls_ssl_config_init(&tlsioInst->config);
    mbedtls_x509_crt_init(&tlsioInst->cacert);	

    if (0 != mbedtls_entropy_add_source(&tlsioInst->entropy,mbedtls_hardware_poll,NULL,128,0))
	{
		result = __LINE__;
	}
	else if (0 != mbedtls_ctr_drbg_seed(&tlsioInst->ctr_drbg,mbedtls_entropy_func,&tlsioInst->entropy,(const unsigned char *)pers,strlen(pers)))
	{
		result = __LINE__;
	}
	else if (0 != mbedtls_ssl_config_defaults(&tlsioInst->config,MBEDTLS_SSL_IS_CLIENT,MBEDTLS_SSL_TRANSPORT_STREAM,MBEDTLS_SSL_PRESET_DEFAULT))
	{
		result = __LINE__;
	}
	else
	{
		mbedtls_ssl_conf_rng(&tlsioInst->config,mbedtls_ctr_drbg_random,&tlsioInst->ctr_drbg);
		mbedtls_ssl_conf_authmode(&tlsioInst->config,MBEDTLS_SSL_VERIFY_REQUIRED);
		mbedtls_ssl_conf_min_version(&tlsioInst->config,MBEDTLS_SSL_MAJOR_VERSION_3,MBEDTLS_SSL_MINOR_VERSION_3);		// v1.2                                                          
		mbedtls_ssl_set_bio(&tlsioInst->ssl,instance,on_io_send,on_io_recv,NULL);
		
		if (0 != (result = mbedtls_ssl_setup(&tlsioInst->ssl,&tlsioInst->config)))
		{
			result = __LINE__;
		}
		else if (0 != mbedtls_ssl_set_hostname(&tlsioInst->ssl,host))
		{
			result = __LINE__;
		}
		else if (0 != (result = mbedtls_ssl_set_session(&tlsioInst->ssl,&tlsioInst->ssn)))
		{
			result = __LINE__;
		}
		else
		{
	#if defined (MBED_TLS_DEBUG_ENABLE)
			mbedtls_ssl_conf_dbg(&tlsioInst->config, mbedtls_debug,stdout);
			mbedtls_debug_set_threshold(5);
	#endif

			// if (0 != mbedtls_ssl_setup(&tlsioInst->ssl,&tlsioInst->config))
			// {
				// result = __LINE__;
			// }
		}
	...
	{
        // mbedTLS cleanup...
        mbedtls_ssl_close_notify(&tlsioInst->ssl);
        mbedtls_ssl_free(&tlsioInst->ssl);
        mbedtls_ssl_config_free(&tlsioInst->config);
        mbedtls_x509_crt_free(&tlsioInst->cacert);
        mbedtls_ctr_drbg_free(&tlsioInst->ctr_drbg);
        mbedtls_entropy_free(&tlsioInst->entropy);
	}
...
OPTIONHANDLER_HANDLE tlsio_mbedtls_retrieveoptions(CONCRETE_IO_HANDLE handle)
{
    ...
    {
		/*this layer currently only cares about the certificates*/
        ...
        if (result != NULL)
        {
            ...
            else
            {
                ...
                {    
                    ...
                    // mbeTLS initialize
                    if (0 != mbedtls_init((void *)result,tls_io_config->hostname))
					{
						LogError("failed to initialize mbedtls");
						result = NULL;
					}
					else
					{
